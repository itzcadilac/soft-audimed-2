...
--changeset lizandro.alipazaga:202502192204
--comment Se crea la tabla guia_ingreso_detalle
CREATE TABLE `guia_ingreso_detalle`  (
  ...
  CONSTRAINT `guia_ingreso_detalle_ibfk_1` FOREIGN KEY (`idguia`) REFERENCES `guia_ingreso` (`idguia`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `guia_ingreso_detalle_ibfk_2` FOREIGN KEY (`idarticulo`) REFERENCES `articulos` (`idarticulo`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;

--rollback DROP TABLE guia_ingreso_detalle;
