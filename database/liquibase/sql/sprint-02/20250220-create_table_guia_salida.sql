...
--changeset lizandro.alipazaga:202502200837
--comment Se crea la tabla guia_salida
CREATE TABLE `guia_salida`  (
  ...
  CONSTRAINT `guia_salida_ibfk_1` FOREIGN KEY (`idalmacen`) REFERENCES `almacen` (`idalmacen`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `guia_salida_ibfk_2` FOREIGN KEY (`idtipomovimiento`) REFERENCES `tipo_movimiento` (`idtipomovimiento`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `guia_salida_ibfk_3` FOREIGN KEY (`idproveedor`) REFERENCES `proveedor` (`idproveedor`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `guia_salida_ibfk_4` FOREIGN KEY (`idtipocomprobante`) REFERENCES `tipo_comprobante` (`idtipocomprobante`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;

--rollback DROP TABLE guia_salida;
